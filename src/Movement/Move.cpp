...
void Move::SetXYBedProbePoint(size_t index, float x, float y)
{
	...
	{
		reprap.GetPlatform().Message(ErrorMessage, "[E303] Z probe point index out of range\n");
	}
	else
	{
...
void Move::SetZBedProbePoint(size_t index, float z, bool wasXyCorrected, bool wasError)
{
	if (index >= MaxProbePoints)
	{
		reprap.GetPlatform().Message(ErrorMessage, "[E304] Z probe point Z index out of range\n");
	}
	else
	{
